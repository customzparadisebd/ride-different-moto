...
import { DEFAULT_STORE_SETTINGS } from "@/lib/settings.shared";
...
function AdminSettings() {
    ...
    const [draft, setDraft] = useState(DEFAULT_STORE_SETTINGS);
    useEffect(() => {
    ...
    const mutation = useMutation({
        ...
        onError: (error) => toast.error(error.message || "Could not save the settings."),
    });
    const togglePayment = (value) => setDraft((current) => ({
        ...current,
    ...
    return (<section className="mx-auto max-w-4xl px-4 pb-20">
      <div className="flex flex-col gap-6 md:flex-row md:items-start">
        {/* Sub-navigation Sidebar */}
        <aside className="w-full shrink-0 md:w-56">
          <nav className="flex gap-1 overflow-x-auto pb-2 md:flex-col md:overflow-visible md:pb-0">
            <SettingsTabButton active={location.pathname === "/ad/settings" && (!search.tab || search.tab === "general")} onClick={() => navigate({ to: "/ad/settings", search: { tab: "general" } })} label="General"/>
            <SettingsTabButton active={location.pathname.includes("/invoice")} onClick={() => navigate({ to: "/ad/settings/invoice" })} label="Invoice"/>
            <SettingsTabButton active={location.pathname === "/ad/settings" && search.tab === "logos"} onClick={() => navigate({ to: "/ad/settings", search: { tab: "logos" } })} label="Logos"/>
            <SettingsTabButton active={location.pathname === "/ad/settings" && search.tab === "delivery"} onClick={() => navigate({ to: "/ad/settings", search: { tab: "delivery" } })} label="Delivery Zones"/>
            <SettingsTabButton active={location.pathname === "/ad/settings" && search.tab === "couriers"} onClick={() => navigate({ to: "/ad/settings", search: { tab: "couriers" } })} label="SteadFast"/>
            <SettingsTabButton active={location.pathname === "/ad/settings" && search.tab === "homepage"} onClick={() => navigate({ to: "/ad/settings", search: { tab: "homepage" } })} label="Homepage"/>
            <SettingsTabButton active={location.pathname === "/ad/settings" && search.tab === "bike-models"} onClick={() => navigate({ to: "/ad/settings", search: { tab: "bike-models" } })} label="Bike Models"/>
            <SettingsTabButton active={location.pathname === "/ad/settings" && search.tab === "gallery"} onClick={() => navigate({ to: "/ad/settings", search: { tab: "gallery" } })} label="Gallery"/>
            <SettingsTabButton active={location.pathname === "/ad/settings" && search.tab === "ai"} onClick={() => navigate({ to: "/ad/settings", search: { tab: "ai" } })} label="AI Tools"/>
          </nav>
        </aside>
        ...
        <div className="flex-1 space-y-8 animate-in fade-in duration-300">
          ...
          {location.pathname === "/ad/settings" && (<>
              {/* TAB: GENERAL */}
              {(!search.tab || search.tab === "general") && (<div className="space-y-8">
                  <div>
                  ...
                  {settingsQuery.isLoading ? (<p className="py-16 text-center text-sm text-muted-foreground">Loading settings…</p>) : (<form className="space-y-6" onSubmit={(event) => {
                        event.preventDefault();
                        mutation.mutate({ data: draft });
                    }}>
                      <fieldset disabled={!canManage || mutation.isPending} className="space-y-6 disabled:opacity-70">
                        <div className="rounded-xl border border-border bg-card p-5 shadow-sm">
                          <h2 className="font-display text-sm font-bold uppercase tracking-wider text-primary">
                            Payment Methods
                          </h2>
                          <div className="mt-4 grid gap-3 sm:grid-cols-2">
                            {PAYMENT_METHODS.map((method) => (<label key={method.value} className="flex min-h-12 items-center gap-3 rounded-lg border border-border bg-background/50 px-4 text-sm transition-colors hover:bg-accent/50 cursor-pointer">
                                <input type="checkbox" className="h-4 w-4 rounded accent-primary" checked={draft.paymentMethods.includes(method.value)} onChange={() => togglePayment(method.value)}/>
                        ...
                        <div className="rounded-xl border border-border bg-card p-5 shadow-sm">
                          ...
                          <div className="mt-4 grid gap-4 sm:grid-cols-3">
                            <div className="space-y-2">
                              ...
                              <Input className="h-11" value={draft.supportPhone} onChange={(event) => setDraft((current) => ({ ...current, supportPhone: event.target.value }))}/>
                            </div>
                            <div className="space-y-2">
                              <Label className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">
                                Support Email
                              </Label>
                              <Input className="h-11" type="email" value={draft.supportEmail ?? ""} onChange={(event) => setDraft((current) => ({ ...current, supportEmail: event.target.value }))}/>
                            </div>
                            <div className="space-y-2">
                              <Label className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">
                                Low-Stock Alert
                              </Label>
                              <Input className="h-11" type="number" value={String(draft.lowStockThreshold)} onChange={(event) => setDraft((current) => ({
                        ...current,
                        ...
                        <div className="rounded-xl border border-border bg-card p-5 shadow-sm">
                          ...
                          <div className="mt-4 grid gap-4 sm:grid-cols-2">
                            <div className="space-y-2">
                              ...
                              <Input className="h-11" value={draft.whatsappPhone} onChange={(event) => setDraft((current) => ({ ...current, whatsappPhone: event.target.value }))}/>
                            </div>
                            <div className="space-y-2">
                              <Label className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">
                                Default Message
                              </Label>
                              <Input className="h-11" value={draft.whatsappMessage} onChange={(event) => setDraft((current) => ({ ...current, whatsappMessage: event.target.value }))}/>
                            </div>
                          </div>
                          ...
                          <div className="mt-6 pt-6 border-t border-border/50 flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
                            <label className="flex items-center gap-3 cursor-pointer">
                              <input type="checkbox" className="h-4 w-4 rounded accent-primary" checked={draft.whatsappFloatingEnabled} onChange={(e) => setDraft((c) => ({ ...c, whatsappFloatingEnabled: e.target.checked }))}/>
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <select className="h-10 rounded-lg border border-input bg-background px-3 text-xs font-bold uppercase tracking-tighter focus:ring-1 focus:ring-primary outline-none" value={draft.whatsappFloatingPosition} onChange={(e) => setDraft((c) => ({ ...c, whatsappFloatingPosition: e.target.value }))}>
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          {!canManage && (<p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">
                              Read-only Access
                            </p>)}
                        </div>
                      </fieldset>
              ...
              {/* TAB: DELIVERY ZONES */}
              {search.tab === "delivery" && (<div className="space-y-8">
                  <div>
                  ...
                  <div className="space-y-6">
                    ...
                    {!canManageZones && (<p className="text-xs text-muted-foreground italic">
              ...
              {/* TAB: STEADFAST */}
              {search.tab === "couriers" && (<div className="space-y-8">
                  <div>
              ...
              {/* TAB: HOMEPAGE */}
              {search.tab === "homepage" && (<div className="space-y-8">
                  <div>
              ...
              {/* TAB: GALLERY */}
              {search.tab === "gallery" && (<GalleryPanel />)}

              {/* TAB: AI TOOLS */}
              {search.tab === "ai" && (<div className="space-y-8">
                  <div>
              ...
              {/* TAB: BIKE MODELS */}
              {search.tab === "bike-models" && (<BikeModelsPanel />)}

              {/* TAB: LOGOS */}
              {search.tab === "logos" && (<LogoSettingsPanel canManage={canManage}/>)}
            </>)}
        </div>
      </div>
    </section>);
