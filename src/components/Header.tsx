  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-[100] border-b border-border bg-background/95 pt-safe shadow-sm backdrop-blur-md supports-[backdrop-filter]:bg-background/80">
      <div className="mx-auto grid h-16 max-w-7xl grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-2 px-3 sm:h-18 sm:px-4 min-[900px]:gap-3 min-[900px]:px-5 min-[1100px]:h-20 min-[1100px]:px-6 xl:px-8">
        ...
        <nav className="hidden min-w-0 items-center justify-center gap-4 overflow-visible min-[768px]:flex min-[820px]:gap-5 min-[900px]:gap-5 min-[1100px]:gap-6 xl:gap-6">
          {navLinks.map((link) => (
            <Link
              key={link.to}
              to={link.to}
              className="min-w-0 whitespace-nowrap px-1 font-display text-[11px] font-bold uppercase leading-snug tracking-[0.16em] text-foreground/80 transition-colors hover:text-primary min-[820px]:text-[12px] min-[900px]:text-[13px] min-[1100px]:px-1.5 min-[1100px]:text-[14px] xl:text-[15px]"
            >
              {link.label}
            </Link>
          ))}
