...
export const Route = createFileRoute("/_authenticated/ad/hero")({
  head: () => ({ meta: [{ title: "Manage Hero Slider — CZP Ops" }] }),
...
function AdminHeroSlides() {
  ...
  if (isLoading) return <div className="p-8 text-center text-muted-foreground">Loading slides...</div>;
        ...
        <div className="rounded-xl border border-primary/20 bg-card p-6 shadow-xl animate-in fade-in slide-in-from-top-4 duration-300">
          ...
          <form onSubmit={handleAdd} className="grid gap-6 sm:grid-cols-2">
            ...
            <div className="space-y-2">
              ...
              <Input id="link_url" name="link_url" placeholder="e.g. /products/visor or all-products" />
            </div>
            <div className="space-y-2">
            ...
            <div className="flex items-end pt-2">
              <Button type="submit" variant="red" className="w-full" disabled={addMutation.isPending}>
          ...
          [...slides].sort((a, b) => a.order - b.order).map((slide) => (
            <div 
              ...
              className={`group relative overflow-hidden rounded-xl border transition-all duration-300 ${
                slide.active ? 'border-border bg-card' : 'border-dashed border-muted-foreground/30 bg-muted/5 opacity-80'
              ...
              {editingId === slide.id ? (
                ...
                >
                  <div className="mb-4 grid gap-6 sm:grid-cols-2">
                    ...
                    <div className="flex items-center gap-4 pt-4">
                      <div className="flex items-center gap-2">
                        <Checkbox id={`active-${slide.id}`} name="is_active" defaultChecked={slide.active} />
                  ...
                  <div className="flex justify-end gap-2 border-t pt-4">
                    ...
                    <Button type="submit" variant="red" size="sm" className="gap-2" disabled={updateMutation.isPending}>
                  ...
                  <div className="relative h-48 w-full shrink-0 overflow-hidden sm:h-auto sm:w-64 bg-black">
                    ...
                    {!slide.active && (
                      <div className="absolute inset-0 flex items-center justify-center bg-black/60 backdrop-blur-sm">
                        <span className="rounded-full bg-white/10 px-3 py-1 text-xs font-bold uppercase tracking-widest text-white border border-white/20">Inactive</span>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex flex-1 flex-col justify-between p-6">
                    <div>
                      <div className="flex items-start justify-between">
                        <div className="space-y-1">
                          <div className="flex items-center gap-2">
                             ...
                             <h3 className="font-display text-xl font-bold uppercase tracking-wide">{slide.bikeName}</h3>
                          </div>
                          {slide.label && <p className="text-sm text-muted-foreground">{slide.label}</p>}
                        </div>
                      ...
                      <div className="mt-4 grid gap-2 text-xs">
                        ...
                        <div className="flex items-center gap-2 text-muted-foreground">
                          <span className="font-semibold uppercase tracking-tighter opacity-50 text-[10px]">Image URL:</span>
