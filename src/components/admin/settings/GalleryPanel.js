...
import { Plus, Trash2, Edit2, Save, X, GripVertical, Info, Image as ImageIcon, CheckCircle2, ArrowUp, ArrowDown, RotateCcw, GalleryThumbnails } from "lucide-react";
...
import { getGalleryItems, createGalleryItem, updateGalleryItem, deleteGalleryItem, reorderGalleryItems } from "@/lib/gallery.functions";
import { SingleImageUpload } from "@/components/admin/SingleImageUpload";
import { Badge } from "@/components/ui/badge";
import { cn } from "@/lib/utils";
import { Alert, AlertDescription, AlertTitle } from "@/components/ui/alert";
function SortableGalleryItem({ id, item, onEdit, onDelete, onMove }) {
    ...
    return (<div ref={setNodeRef} style={style} className={cn("group overflow-hidden rounded-lg border border-border bg-card transition-all duration-200", isDragging ? "border-primary shadow-2xl ring-2 ring-primary/20 scale-[1.02] opacity-90" : "hover:border-primary/30")}>
      ...
      <div className="p-4 sm:p-5 flex flex-col sm:flex-row gap-5 items-center">
        <button {...attributes} {...listeners} className="cursor-grab active:cursor-grabbing text-muted-foreground hover:text-primary p-2 rounded-md hover:bg-primary/5 transition-colors" title="Drag to reorder">
        ...
        <div className="relative aspect-square w-full sm:w-32 shrink-0 overflow-hidden rounded-md border border-border bg-muted">
          {item.image ? (<img src={item.image} alt={item.alt} className="h-full w-full object-cover transition-transform group-hover:scale-105"/>) : (<div className="flex h-full items-center justify-center text-muted-foreground/40">
              <ImageIcon className="h-6 w-6"/>
            </div>)}
          {!item.active && (<div className="absolute inset-0 bg-black/60 flex items-center justify-center">
        ...
        <div className="flex items-center gap-1.5 bg-accent/20 p-1.5 rounded-lg border border-border/50">
          <div className="flex flex-col gap-1">
            <Button variant="ghost" size="icon" className="h-8 w-8 hover:text-primary hover:bg-primary/10" onClick={(e) => { e.stopPropagation(); onMove("up"); }} title="Move Up">
              <ArrowUp className="h-4 w-4"/>
            </Button>
            <Button variant="ghost" size="icon" className="h-8 w-8 hover:text-primary hover:bg-primary/10" onClick={(e) => { e.stopPropagation(); onMove("down"); }} title="Move Down">
              <ArrowDown className="h-4 w-4"/>
            </Button>
          </div>
          <div className="w-[1px] h-10 bg-border/50 mx-1"/>
          <Button variant="ghost" size="icon" className="h-9 w-9 text-muted-foreground hover:text-primary hover:bg-primary/10" onClick={onEdit} title="Edit Details">
            <Edit2 className="h-4 w-4"/>
          </Button>
          <Button variant="ghost" size="icon" className="h-9 w-9 text-muted-foreground hover:text-destructive hover:bg-destructive/10" onClick={onDelete} title="Delete Image">
...
export function GalleryPanel() {
    ...
    const deleteMutation = useMutation({
        mutationFn: (id) => removeItem({ data: id }),
        onSuccess: () => {
    ...
    if (isLoading)
        return <div className="p-12 text-center text-muted-foreground animate-pulse">Loading gallery...</div>;
    return (<div className="space-y-6">
      <div className="flex items-center justify-between sticky top-0 z-20 py-4 bg-background/80 backdrop-blur-md border-b border-border/50">
        ...
        <div className="flex gap-3">
          {isDirty && (<>
              <Button onClick={() => { setItems(rawItems); setIsDirty(false); }} variant="outline" size="sm" className="gap-2">
                <RotateCcw className="h-4 w-4"/> Reset
              </Button>
              <Button onClick={saveOrder} variant="onyx" size="sm" className="gap-2 shadow-lg shadow-black/20">
      ...
      <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd} modifiers={[restrictToVerticalAxis]}>
        <SortableContext items={items.map((m) => m.id)} strategy={verticalListSortingStrategy}>
          <div className="space-y-4">
            {items.map((m) => (<SortableGalleryItem key={m.id} id={m.id} item={m} onEdit={() => setEditingId(m.id)} onDelete={() => {
                if (confirm(`Are you sure you want to permanently delete this gallery image?`)) {
                    deleteMutation.mutate(m.id);
                }
            }} onMove={(dir) => moveItem(m.id, dir)}/>))}
          </div>
        </SortableContext>
      </DndContext>
      
      {isAdding && (<GalleryItemForm onSave={(d) => addMutation.mutate(d)} onCancel={() => setIsAdding(false)} isSaving={addMutation.isPending}/>)}
      
      {editingId && (<GalleryItemForm initialData={items.find(m => m.id === editingId)} onSave={(updates) => updateMutation.mutate({ id: editingId, updates })} onCancel={() => setEditingId(null)} isSaving={updateMutation.isPending}/>)}

      {items.length === 0 && !isAdding && (<div className="text-center py-20 border-2 border-dashed border-border rounded-2xl bg-accent/20">
...
function GalleryItemForm({ initialData, onSave, onCancel, isSaving }) {
    const [formData, setFormData] = useState({
        image_url: initialData?.image || "",
        alt_text: initialData?.alt || "",
        is_active: initialData?.active ?? true,
    });
    const [imageMode, setImageMode] = useState(initialData?.image?.includes("supabase.co") ? "upload" : "url");
    return (<div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      <Card className="w-full max-w-lg border-primary/20 shadow-2xl overflow-hidden">
        ...
        <CardContent className="pt-6 space-y-6">
          <div className="space-y-4">
            ...
            {imageMode === "upload" ? (<SingleImageUpload value={formData.image_url} onChange={(url) => setFormData({ ...formData, image_url: url })} bucket="products" pathPrefix="gallery" guideline="800x800 (1:1) recommended"/>) : (<Input value={formData.image_url} onChange={(e) => setFormData({ ...formData, image_url: e.target.value })} placeholder="Image URL"/>)}
