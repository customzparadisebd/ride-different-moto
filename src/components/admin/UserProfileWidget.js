...
import { Camera, Loader2, Upload, Pencil, X, Check, Info, ChevronRight, Scissors } from "lucide-react";
...
export function UserProfileWidget({ access }) {
    ...
    const roleLabel = access.primaryRole ? ROLE_LABELS[access.primaryRole] : "Staff";
    ...
    return (<>
      <div className="flex items-center gap-3 px-2 py-3 cursor-pointer hover:bg-muted/50 rounded-lg transition-colors group" onClick={() => setIsDialogOpen(true)}>
        <div className="relative shrink-0">
          <div className="h-10 w-10 rounded-full border-2 border-primary/20 overflow-hidden bg-muted flex items-center justify-center shadow-inner">
            <img src={currentAvatar} alt="Avatar" className="h-full w-full object-cover transition-transform group-hover:scale-110" key={currentAvatar}/>
          </div>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="sm:max-w-[500px] p-0 overflow-hidden bg-background border-border shadow-2xl">
            ...
            <div className="flex flex-col items-center justify-center gap-4">
              <div className="relative">
                <div className="size-24 rounded-full border-4 border-primary/10 overflow-hidden bg-muted flex items-center justify-center shadow-xl transition-all duration-500 hover:border-primary/30">
                  <img src={previewUrl || currentAvatar} alt="Avatar Preview" className="h-full w-full object-cover"/>
                  {isUpdating && (<div className="absolute inset-0 bg-background/60 backdrop-blur-[2px] flex items-center justify-center">
                      <Loader2 className="h-8 w-8 animate-spin text-primary"/>
                    </div>)}
                </div>
                <div className="absolute -bottom-1 -right-1 flex gap-1">
                  {access.avatarUrl && (<Button variant="destructive" size="icon" className="h-7 w-7 rounded-full shadow-lg border-2 border-background" onClick={handleResetAvatar} disabled={isUpdating} title="Remove custom avatar">
                      <X className="h-3.5 w-3.5"/>
                    </Button>)}
                  <Button variant="steel" size="icon" className="h-7 w-7 rounded-full shadow-lg border-2 border-background" onClick={() => fileInputRef.current?.click()} disabled={isUpdating} title="Change image">
            ...
            <Tabs defaultValue="presets" className="w-full">
              ...
              <TabsContent value="presets" className="pt-4 animate-in fade-in slide-in-from-bottom-2">
                <ScrollArea className="h-56 pr-4">
                  <div className="grid grid-cols-5 gap-3">
                    {PRESET_AVATARS.map((avatar) => (<button key={avatar.id} onClick={() => handleSelectPreset(avatar.url)} disabled={isUpdating} className={cn("group relative aspect-square rounded-lg border-2 overflow-hidden transition-all hover:border-primary/50 hover:scale-105 active:scale-95 bg-muted/20", access.avatarUrl === avatar.url ? "border-primary bg-primary/10 shadow-lg" : "border-transparent")} title={avatar.label}>
                        <img src={avatar.url} alt={avatar.label} className="w-full h-full object-cover"/>
                        {access.avatarUrl === avatar.url && (<div className="absolute inset-0 bg-primary/20 flex items-center justify-center">
              ...
              <TabsContent value="upload" className="pt-4 space-y-4 animate-in fade-in slide-in-from-bottom-2">
                <div className="rounded-lg border border-dashed border-border bg-muted/20 p-8 flex flex-col items-center justify-center gap-3 transition-colors hover:bg-muted/30">
                  ...
                  <Button variant="steel" size="sm" className="mt-2 font-bold uppercase tracking-widest text-[10px]" disabled={isUpdating} onClick={() => fileInputRef.current?.click()}>
                    Select Image
                  </Button>
                </div>
          ...
          <DialogFooter className="p-4 bg-muted/30 border-t border-border gap-2">
            <Button variant="steel" className="text-[10px] font-bold uppercase tracking-widest w-full sm:w-auto" onClick={() => setIsDialogOpen(false)}>
              Close
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={isCropOpen} onOpenChange={setIsCropOpen}>
        <DialogContent className="sm:max-w-[500px] bg-background border-border p-0 overflow-hidden shadow-2xl">
          ...
          <div className="relative h-80 bg-neutral-900">
            {cropImage && (<Cropper image={cropImage} crop={crop} zoom={zoom} aspect={1} onCropChange={setCrop} onCropComplete={onCropComplete} onZoomChange={setZoom} cropShape="round" showGrid={false}/>)}
          </div>

          <div className="p-6 space-y-4">
            <div className="space-y-3">
              <Label className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground">Zoom Level</Label>
              <Slider value={[zoom]} min={1} max={3} step={0.1} onValueChange={(values) => {
            const val = values[0];
          ...
          <DialogFooter className="p-4 bg-muted/30 border-t border-border flex gap-2">
            <Button variant="ghost" className="flex-1 text-[10px] font-bold uppercase tracking-widest" onClick={() => {
            setIsCropOpen(false);
            setCropImage(null);
        }} disabled={isUpdating}>
              Cancel
            </Button>
            <Button variant="red" className="flex-1 text-[10px] font-bold uppercase tracking-widest shadow-lg" onClick={handleCropSave} disabled={isUpdating}>
              {isUpdating ? (<>
...
function Badge({ children, className, variant = "default" }) {
    return (<span className={cn("inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ring-1 ring-inset", variant === "outline" ? "ring-border text-foreground" : "bg-primary/10 text-primary ring-primary/20", className)}>
      {children}
    </span>);
}
